...
def plant_trees(feature_name, n_tree=1, criterion="entropy"):
    global Feature_Data, Label_Data
    check_data()
    ...
    for _ in tqdm(range(n_tree), desc="Plant trees"):
        ...
        dt_m = dt.fit(Feature_Data, Label_Data)
...
def val_trees(tree_eval, pass_rate=0.975, class_importance=None):
    ...
    logging.info("{0}% misclassification rules are learned.".format(round(t0_avg * 100, 2)))
    logging.info("{0}% positive classification rules are learned.".format(round(t1_avg * 100, 2)))
    if good_rate >= pass_rate:
        logging.info("Trees are validated and pass")
        return True, [good_rate, (t0_avg, t0_range), (t1_avg, t1_range)]
    else:
        logging.warning(
            "Validation rate is not satisfying the pass requirements. Required: {0};  Current: {1}".format(pass_rate,
                                                                                                           round(
                                                                                                               good_rate,
                                                                                                               3)))
...
def tree_decision(n, t, f):
    ...
    return decision, distinguish, (direction_t, direction_f), (direction_m, direction_c), decision_tf, decision_status
...
def analyse_trees(path, node, error, feature, round_num=0, degree_threshold=(0.25, 0.5), own_result=None):
    ...
    for i in tqdm(range(len(node)), desc="Analyse trees"):
        ...
        for j in node[i].keys():
            ...
            decision, distinguish, direction_tf, direction_mc, decision_tf, decision_status = \
            ...
            if round_num > 0:
                ...
                direction_mc = (round(direction_mc[0], round_num), round(direction_mc[1], round_num))
...
def threshold_split_mis(result, feature_range):
    lower = []
    upper = []
    ...
    for i in re_t:
        if i["direction_mc"][0] * i["distinguish"] * i["usage"] < 0:
            upper.append((i["threshold"], abs(i["direction_mc"][0] * i["distinguish"] * i["usage"]) / 2))
        else:
            upper.append((i["threshold"], i["direction_mc"][0] * i["distinguish"] * i["usage"]))
    for i in re_f:
        if (0 - i["direction_mc"][0]) * i["distinguish"] * i["usage"] < 0:
            lower.append((i["threshold"], abs((0 - i["direction_mc"][0]) * i["distinguish"] * i["usage"]) / 2))
        else:
            lower.append((i["threshold"], (0 - i["direction_mc"][0]) * i["distinguish"] * i["usage"]))
...
def bound_range(lower, upper, top_n, max_move=1):
    ...
    for i in range(top_n):
        ...
        if abs(upper_gap) > max_move / r[0]:
            if upper_gap > 0:
                upper_gap = max_move / r[0]
            else:
                upper_gap = 0 - max_move / r[0]
...
def c_s_overall(cs_list, c_s):
    overall = 0
    for i in cs_list:
        count = i[0] / max([j[0] for j in cs_list])
...
def summary_trees(tree_result, feature, route, node, feature_range, feature_weight="balance", status_bonus=None,
    ...
    for tree in tqdm(range(len(tree_result)), desc="Summarise trees - Brief Report"):
        ...
        for f in feature:
            ...
            lower, upper = threshold_split_mis(tree_result[tree][f], feature_range[f])
    ...
    for tree in tqdm(range(len(tree_result)), desc="Summarise trees - Route Report"):
        ...
        for r in route[tree]:
            ...
            decision_r = node[tree][r[-1][0]]["value"][0] >= node[tree][r[-1][0]]["value"][1]
            ...
            if decision_r:
                route_mis.append(result_dir)
            else:
                route_cls.append(result_dir)

    return report, route_mis, route_cls, node
...
def explain_tree(report, route_mis, route_cls, node, feature_range, route_top=5, stable_move=0.75, save=None):
    ...
    output_text += "The summary section will show the importance of each feature.\n" \
                   ...
                   "* Please refer the range and value to determine the mis-classification reasons\n=============\n\n"
    for i in sorted(report.values(), key=lambda x: x["rank"]):
        ...
        desc = "Mis-classification might be happened when "
        ...
        output_text += "Rank: {rank} - Feature: {feature} - Overall Score: {overall_norm} ({overall}) " \
                       "- Count: {count} - Importance Score: {score_norm} ({score}) - Range: {rang} " \
    ...
    route_mis_overall = sorted(route_mis, key=lambda x: x["overall"], reverse=True)
    route_cls_overall = sorted(route_cls, key=lambda x: x["overall"], reverse=True)
    output_text += "\n===Route Score Rank===\n"
    count = 0
    output_text += "\n-----Mis-classification-----\n"
    for i in route_mis_overall[:route_top]:
        output_text += "Rank: {rank} - Overall Score: {overall} - Mis-classified?: {decision} - Value: {value} - " \
    ...
    for i in route_cls_overall[:route_top]:
        output_text += "Rank: {rank} - Overall Score: {overall} - Mis-classified?: {decision} - Value: {value} - " \
                       "Length: {length}\nRoute: ".format(rank=count, overall=i["overall"], decision=i["decision"],
                                                          value=i["value"][0], length=i["length"])
        output_text += write_route(i["feature"], i["threshold"], i["tf"]) + "\n"
    ...
    count = 0
    output_text += "\n-----Mis-classification-----\n"
    route_mis_len = sorted(route_mis, key=lambda x: x["length"], reverse=False)
    route_cls_len = sorted(route_cls, key=lambda x: x["length"], reverse=False)
    for i in route_mis_len[:route_top]:
        output_text += "Rank: {rank} - Overall Score: {overall} - Mis-classified?: {decision} - Value: {value} - " \
                       "Length: {length}\nRoute: ".format(rank=count, overall=i["overall"], decision=i["decision"],
                                                          value=i["value"][0], length=i["length"])
        output_text += write_route(i["feature"], i["threshold"], i["tf"]) + "\n"
    ...
    count = 0
    output_text += "\n-----Correct Classification-----\n"
    for i in route_cls_len[:route_top]:
        output_text += "Rank: {rank} - Overall Score: {overall} - Mis-classified?: {decision} - Value: {value} - " \
                       "Length: {length}\nRoute: ".format(rank=count, overall=i["overall"], decision=i["decision"],
                                                          value=i["value"][0], length=i["length"])
        output_text += write_route(i["feature"], i["threshold"], i["tf"]) + "\n"
        ...
        count += 1
    output_text += "\n===Route Value===\n"
    route_mis_value = sorted(route_mis, key=lambda x: x["value"][0], reverse=True)
    route_cls_value = sorted(route_cls, key=lambda x: x["value"][1], reverse=True)
    count = 0
    output_text += "\n-----Mis-classification-----\n"
    for i in route_mis_value[:route_top]:
        output_text += "Rank: {rank} - Overall Score: {overall} - Mis-classified?: {decision} - Value: {value} - " \
                       "Length: {length}\nRoute: ".format(rank=count, overall=i["overall"], decision=i["decision"],
                                                          value=i["value"][0], length=i["length"])
        output_text += write_route(i["feature"], i["threshold"], i["tf"]) + "\n"
    ...
    count = 0
    output_text += "\n-----Correct Classification-----\n"
    for i in route_cls_value[:route_top]:
        output_text += "Rank: {rank} - Overall Score: {overall} - Mis-classified?: {decision} - Value: {value} - " \
                       "Length: {length}\nRoute: ".format(rank=count, overall=i["overall"], decision=i["decision"],
                                                          value=i["value"][0], length=i["length"])
        output_text += write_route(i["feature"], i["threshold"], i["tf"]) + "\n"
